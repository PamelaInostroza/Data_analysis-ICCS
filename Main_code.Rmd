...
e. Latin American Countries: Chile, Peru.  
...
ISGC2 <- ISGC2 %>% rename_at(vars(c("TOTWGTS","SENWGTS")), ~ c("TOTWGT_Gc2", "SENWGT_Gc2"))
...
# ICCS %>% dplyr::select(TOTWGT_Gc1,SENWGT_Gc1, TOTWGT_Gc2,SENWGT_Gc2, TOTWGT_Gc3,SENWGT_Gc3,COUNTRY, cycle, questG, questL, questE) %>% 
#   group_by(COUNTRY, cycle, questG, questL, questE) %>% 
#   summarise(w99 = sum(TOTWGT_Gc1, na.rm = TRUE),
#             w99s =sum(SENWGT_Gc1, na.rm = TRUE),
...
```{r cfa, out.width = '100%', out.height='100%', results = 'asis'}
