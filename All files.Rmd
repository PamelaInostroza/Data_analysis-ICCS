...
# Read SPSS files to R and Joining them

Select countries and years (cycle) to analyse 
...
Countries <- read_xlsx("DataAnalysis/Metadata.xlsx", sheet = "CNT")
Files <- read_xlsx("DataAnalysis/Metadata.xlsx", sheet = "Files")


CNT = c("ITA","CHL") #,"BFL","PER"

years <- Countries %>% filter(AlphaCode %in% CNT & !is.na(Part)) %>% select(Country, AlphaCode, year) 
...
for (cnt in CNT) {
  ...
  yearf <- years %>% filter(AlphaCode == cnt) %>% arrange(as.numeric(year))
  ...
  for (year in yearf$year) {
    ...
    CountryEU <- Countries %>% filter(year == year & !is.na(EUQ)) %>% select(AlphaCode)
    CountryLA <- Countries %>% filter(year == year & !is.na(LAQ)) %>% select(AlphaCode) 
    File <- Files %>% filter(!is.na(ToUse)) %>% select(Filename)
...
tabISG3 <- read_xlsx("DataAnalysis/ICCS2016MS_Codebook.xlsx", sheet = "ISGC3")
tabISG3 <- tabISG3 %>% select(ID,VariableC3, VariableC2, VariableC1, Domain, Construct, Label, `Value Scheme Detailed`) %>% mutate(Dataset = "ISG")
tabISE3 <- read_xlsx("DataAnalysis/ICCS2016MS_Codebook.xlsx", sheet = "ISEC3")
tabISE3 <- tabISE3 %>% select(ID,VariableC3, VariableC2, VariableC1, Domain, Construct, Label, `Value Scheme Detailed`) %>% mutate(Dataset = "ISE")
tabISL3 <- read_xlsx("DataAnalysis/ICCS2016MS_Codebook.xlsx", sheet = "ISLC3")
tabISL3 <- tabISL3 %>% select(ID,VariableC3, VariableC2, VariableC1, Domain, Construct, Label, `Value Scheme Detailed`) %>% mutate(Dataset = "ISL")
tabITG3 <- read_xlsx("DataAnalysis/ICCS2016MS_Codebook.xlsx", sheet = "ITGC3")
tabITG3 <- tabITG3 %>% select(ID,VariableC3, VariableC2, VariableC1, Domain, Construct, Label, `Value Scheme Detailed`) %>% mutate(Dataset = "ITG")
tabICG3 <- read_xlsx("DataAnalysis/ICCS2016MS_Codebook.xlsx", sheet = "ICGC3")
tabICG3 <- tabICG3 %>% select(ID,VariableC3, VariableC2, VariableC1, Domain, Construct, Label, `Value Scheme Detailed`) %>% mutate(Dataset = "ICG")

VarsToUse <- tabISG3 %>% rbind(tabISE3) %>% rbind(tabISL3) %>% rbind(tabITG3) %>% rbind(tabICG3)
#save(VarsToUse, file = "DataAnalysis/Codebook.RData")
VarsToUse <- VarsToUse %>% filter(!is.na(Construct))
rm("tabISG3", "tabISE3", "tabISL3", "tabITG3", "tabICG3")


idS <- VarsToUse %>% filter(Dataset == "ISG" & Construct == "ID Variables" & !VariableC3 %in% c("IDGRADE", "IDPOP", "IDBOOK")) %>% select(VariableC3) %>%  pull()
idT <- VarsToUse %>% filter(Dataset == "ITG" & Construct == "ID Variables" & !VariableC3 %in% c("IDGRADE")) %>% select(VariableC3) %>%  pull()
idC <- VarsToUse %>% filter(Dataset == "ICG" & Construct == "ID Variables" & !VariableC3 %in% c("IDGRADE")) %>% select(VariableC3) %>%  pull()
...
ISC1 <- ISGC1 
ISGC1json <- toJSON(unname(split(ISGC1, 1:nrow(ISGC1))))
names(ISGC1json)
all.equal(ISC1, fromJSON(ISGC1json))

ISGC2json <- toJSON(unname(split(ISGC2, 1:nrow(ISGC2))))
cat(ISGC2json)
all.equal(ISGC2, fromJSON(ISGC2json))
```

```{r mergedata}
#Merging student questionnaires same cycle, General + European + Latin American
varsC1 <- VarsToUse %>% filter(!is.na(VariableC1)) %>% select(VariableC1) %>% pull()
ISC1 <- ISGC1[, colnames(ISGC1) %in% varsC1]

varsC2 <- VarsToUse %>% filter(!is.na(VariableC2)) %>% select(VariableC2) %>% pull()
ISC2 <- ISGC2[, colnames(ISGC2) %in% c(varsC2, "cycle")] %>% 
  full_join(ISEC2[, colnames(ISEC2) %in% c(varsC2, "cycle")], by = c("cycle", idS)) %>% 
  full_join(ISLC2[, colnames(ISLC2) %in% c(varsC2, "cycle")], by = c("cycle", idS)) 

varsC3 <- VarsToUse %>% filter(!is.na(VariableC3)) %>% select(VariableC3) %>% pull()
ISC3 <- ISGC3[, colnames(ISGC3) %in% c(varsC3, "cycle")] %>% 
  full_join(ISEC3[, colnames(ISEC3) %in% c(varsC3, "cycle")], by = c("cycle", idS)) %>% 
  full_join(ISLC3[, colnames(ISLC3) %in% c(varsC3, "cycle")], by = c("cycle", idS)) 
...
ITG <- ITGC1[, colnames(ITGC1) %in% c(varsC1, "cycle")] %>%
  bind_rows(ITGC2_r[, colnames(ITGC2_r) %in% c(varsC2, "cycle")]) %>%
  bind_rows(ITGC3_r[, colnames(ITGC3_r) %in% c(varsC3, "cycle")]) #%>% 
...
ICG <- ICGC1[, colnames(ICGC1) %in% c(varsC1, "cycle")] %>%
  bind_rows(ICGC2_r[, colnames(ICGC2_r) %in% c(varsC2, "cycle")]) %>%
  bind_rows(ICGC3_r[, colnames(ICGC3_r) %in% c(varsC3, "cycle")]) #%>% 
...
ISC <- ISC1[, colnames(ISC1) %in% c(varsC1, "cycle")] %>%
  bind_rows(ISC2_r[, colnames(ISC2_r) %in% c(varsC2, "cycle")]) %>%
  bind_rows(ISC3_r[, colnames(ISC3_r) %in% c(varsC3, "cycle")]) #%>% 
