...
CNTd <- c("ITA", "MLT")
CNTe <- c("CHL", "PER")
CNT <- CNTb #c(CNTa, CNTb, CNTc, CNTd, CNTe)
# c(AUS", "BFL", "BGR", "CHE", "CHL", "COL", "CYP", "CZE", "DEU", "DNK", "DOM", "ENG", "ESP", "EST", "FIN", "FRA", "GRC", "HKG", "HRV", "HUN", "ISR", "ITA", "KOR", "LTU", "LVA", "MEX", "MLT", "NLD", "NOR", "PER", "POL", "PRT", "ROM", "RUS", "SRB", "SVK", "SVN", "SWE", "TWN", "USA")
...
if (is.data.frame(ISGC1)) ISC1 <- ISGC1[, colnames(ISGC1) %in% varsC1]
...
if (!is.data.frame(ITGC1)) ITGC1 <- NULL else ITGC1 <- ITGC1[, colnames(ITGC1) %in% c(varsC1, "cycle")]
if (!is.data.frame(ITGC2)) ITGC2 <- NULL else ITGC2 <- ITGC2[, colnames(ITGC2) %in% c(varsC2, "cycle")]
if (!is.data.frame(ITGC3)) ITGC3 <- NULL else ITGC3 <- ITGC3[, colnames(ITGC3) %in% c(varsC3, "cycle")]
...
if (!is.data.frame(ICGC1)) ICGC1 <- NULL else ICGC1 <- ICGC1[, colnames(ICGC1) %in% c(varsC1, "cycle")]
if (!is.data.frame(ICGC2)) ICGC2 <- NULL else ICGC2 <- ICGC2[, colnames(ICGC2) %in% c(varsC2, "cycle")]
if (!is.data.frame(ICGC3)) ICGC3 <- NULL else ICGC3 <- ICGC3[, colnames(ICGC3) %in% c(varsC3, "cycle")]
...
#plot()
#/*<img src="Main_code_files/figure-html/sub_chunk_cycle1item1Const1-1.png" width="864" />*/
```
...
```{r cfa, out.width = '80%', results = 'asis'}
