...
if(!any(colnames(ISC) %in% c("BS4G6_r","BS4G9_r","BS4G13_r","IS2P24C_r","IS2P24D_r","IS2P24F_r","IS3G24C_r","IS3G24D_r","IS3G24F_r"))){
...
if (any(grepl("99", years$year))){
  ...
  ds991 <- ISC %>% filter(!is.na(TOTWGT_Gc1)) %>% dplyr::select(all_of(index99), IDSTUD, IDSCHOOL, SENWGT_Gc1, COUNTRY, GENDER) %>% 
    mutate(GENDER = as.character(GENDER)) 
  ds99 <- ds991 %>% mutate_at(.funs = as.numeric, .vars = index99)

  survey.design99 <- svydesign(ids=~IDSCHOOL, weights=~SENWGT_Gc1, data=ds99)
  
  cfa99 <- cfa(model99, data = ds99, cluster = "IDSCHOOL", missing = "fiml")
  ...
  p99 <- p99 %>% mutate(cycle = "C1") %>% dplyr::select(cycle, IDSTUD, IDSCHOOL, COUNTRY,Gend_Equal, Immi_Equal, Ethn_Equal)
  
  cnt99 <- unique(ds99$COUNTRY)
  meast99 <- NULL
  stdl99 <- NULL
  for (c99 in cnt99) {
    survey.cnt99 <- svydesign(ids=~IDSCHOOL, weights=~SENWGT_Gc1, data=ds99[ds99$COUNTRY == c99,])
    
    CNTcfa <- cfa(model99, data = ds99[ds99$COUNTRY == c99,], cluster = "IDSCHOOL", missing = "fiml")
...
if (any(grepl("09", years$year))){
  ...
  ds091 <- ISC %>% filter(!is.na(TOTWGT_Gc2)) %>% dplyr::select(all_of(index09), IDSTUD, IDSCHOOL,TOTWGT_Gc2, SENWGT_Gc2, COUNTRY, SGENDER) %>% 
    mutate(SGENDER = as.character(SGENDER)) 
  ds09 <- ds091 %>% mutate_at(.funs = as.numeric, .vars = index09) 
  
  survey.design09 <- svydesign(ids=~IDSCHOOL, weights=~SENWGT_Gc2, data=ds09)
  
  cfa09 <- cfa(model09, data = ds09, cluster = "IDSCHOOL", missing = "fiml")
  ...
  p09 <- p09 %>% mutate(cycle = "C2") %>% dplyr::select(cycle, IDSTUD, IDSCHOOL, COUNTRY,Gend_Equal, Immi_Equal, Ethn_Equal)
  
  cnt09 <- unique(ds09$COUNTRY)
  meast09 <- NULL
  stdl09 <- NULL
  for (c09 in cnt09) {
    survey.cnt09 <- svydesign(ids=~IDSCHOOL, weights=~SENWGT_Gc2, data=ds09[ds09$COUNTRY == c09,])
...
if (any(grepl("16", years$year))){
  ...
  stdl16 <- NULL
  for (c16 in cnt16) {
    survey.cnt16 <- svydesign(ids=~IDSCHOOL, weights=~SENWGT_Gc3, data=ds16[ds16$COUNTRY == c16,])
  ...
  cat('  \n')
  cat('  \n')
  inv.conf16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "COUNTRY", missing = "fiml")
  inv.conf16 <- lavaan.survey(lavaan.fit = inv.conf16, survey.design = survey.design16, estimator= "MLMVS")
  inv.metr16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "COUNTRY", group.equal = c("loadings"), missing = "fiml")
  inv.metr16 <- lavaan.survey(lavaan.fit = inv.metr16, survey.design = survey.design16, estimator= "MLMVS")
  inv.scal16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "COUNTRY", group.equal = c("loadings","intercepts"), missing = "fiml")
  inv.scal16 <- lavaan.survey(lavaan.fit = inv.scal16, survey.design = survey.design16, estimator= "MLMVS")
  inv.stri16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "COUNTRY", group.equal = c("loadings","intercepts","lv.variances"), missing = "fiml")
  ...
  cat('  \n')
  cat('  \n')
  inv.conf16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "S_GENDER", missing = "fiml")
  inv.conf16 <- lavaan.survey(lavaan.fit = inv.conf16, survey.design = survey.design16, estimator= "MLMVS")
  inv.metr16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "S_GENDER", group.equal = c("loadings"), missing = "fiml")
  inv.metr16 <- lavaan.survey(lavaan.fit = inv.metr16, survey.design = survey.design16, estimator= "MLMVS")
  inv.scal16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "S_GENDER", group.equal = c("loadings","intercepts"), missing = "fiml")
  inv.scal16 <- lavaan.survey(lavaan.fit = inv.scal16, survey.design = survey.design16, estimator= "MLMVS")
  inv.stri16 <- cfa(model16, data = ds16, cluster = "IDSCHOOL", group = "S_GENDER", group.equal = c("loadings","intercepts","lv.variances"), missing = "fiml")
  inv.stri16 <- lavaan.survey(lavaan.fit = inv.stri16, survey.design = survey.design16, estimator= "MLMVS")
...
s1 <- ISC_lv %>% dplyr::select(cycle, COUNTRY, Gend_Equal) %>% na.omit() %>% 
  ...
  theme(legend.position = "none")
print(s1)
s2 <- ISC_lv %>% dplyr::select(cycle, COUNTRY, Immi_Equal) %>% na.omit() %>% 
  ...
  theme(legend.position = "none")
print(s2)
s3 <- ISC_lv %>% dplyr::select(cycle, COUNTRY, Ethn_Equal) %>% na.omit() %>% 
